...
function incorrect_args(){
  echo "Usage:
        $0 -e <EC Service Name> -c <Client ID> -s <Client Secret> -d <Refresh token duration> -r <Resource Host> -p <Resource Port#> -l <Local Port>  [-x <Proxy URL> -y <Flag to push to Predix>]"
}
...
sst=$(echo $raw_ec_data | jq -r '."service-uri"')
...
sleep 5
./ecagent_linux_sys -mod "server" -zon "$zon" -sst "$sst" -aid "$id_1" -cid "$cid" -csc "$csc" -dur "$dur" -hst "$hst" -oa2 "$oa2" -rht "$rht" -rpt "$rpt" -hca "\${PORT}" -dbg
...
#!/bin/bash
./ecagent_darwin_sys -mod "client" -aid "$id_2" -tid "$id_1" -hst "$hst" -lpt "$lpt" -cid "$cid" -csc "$csc" -oa2 "$oa2" -dur "$dur" -zon "$zon" -dbg $pxy
EOL
...
echo "./ecagent_darwin_sys -mod client -aid $id_2 -tid $id_1 -hst $hst -lpt $lpt -cid $cid -csc $csc -oa2 $oa2 -dur $dur -zon $zon -dbg $pxy"
echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
